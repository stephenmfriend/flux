...
import { setStorageAdapter, initStore, STATUSES, AGENTS, PRIORITIES, WEBHOOK_EVENT_TYPES, type Guardrail, type Agent, type Priority } from '@flux/shared';
        ...
        inputSchema: {
          ...
          properties: {
            ...
            priority: {
              type: 'integer',
              enum: PRIORITIES,
              description: 'Priority level: 0 = urgent (P0), 1 = normal (P1), 2 = low (P2)',
            },
            agent: {
              type: 'string',
              enum: AGENTS,
              description: 'Agent assignment (claude, codex, gemini, other)',
            },
            acceptance_criteria: {
              type: 'array',
              items: { type: 'string' },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'create_task': {
      ...
      // Set agent via update if provided (createTask options don't include agent)
      if (args?.agent) {
        await updateTask(task.id, { agent: args.agent as Agent });
      }
      return {
