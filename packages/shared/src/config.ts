...
export type FluxConfig = {
  ...
  dataFile?: string;  // Local data file path (file mode) - legacy, use storage instead
  project?: string;   // Default project ID
  storage?: StorageProviderConfig;  // Storage provider configuration (new)
};
...
// Load .env.local from .flux directory
...
export function readConfig(fluxDir: string): FluxConfig {
  ...
  // Expand environment variables in storage.options if present
  let storage = raw.storage;
  if (storage?.options) {
    const expandedOptions: Record<string, any> = {};
    for (const [key, value] of Object.entries(storage.options)) {
      if (typeof value === 'string') {
        expandedOptions[key] = expandEnv(value) || value;
      } else {
